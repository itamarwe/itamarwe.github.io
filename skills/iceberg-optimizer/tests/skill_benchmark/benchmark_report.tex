...
\begin{titlepage}
  ...
  \begin{tabular}{ll}
    ...
    \textbf{Overall}   & \pass\ --- 22/22 scenarios, avg judge score 4.86/5 \\
    \bottomrule
  \end{tabular}
\end{titlepage}
...
Version~3 expands coverage to \textbf{22 scenarios} and achieves an average judge
score of \textbf{4.86/5} with a \textbf{100\% pass rate} (22/22 PASS).  Two
scenarios scored below 5/5: \code{flink\_micro\_commit\_scatter} (3/5 --- correct
root-cause diagnosis but included z-order which the expected outcome says to omit)
and \code{late\_arriving\_data} (4/5 --- correct compaction strategy but did not
explicitly note that write-time sort order K was already set).
...
flink\_micro\_commit\_scatter     & \score{3} & Correctly prioritises \code{distribution-mode=hash} (Action~J) first; produced all required actions but also included z-order which the expected outcome says to omit. \\[3pt]
late\_arriving\_data              & \score{4} & Correctly prescribes sort compaction on recently-modified partitions; minor gap in not explicitly noting that K is already set and should not be added again. \\[3pt]
...
\midrule
\textbf{Total} & \textbf{4.86\,/\,5} & 22/22 scenarios PASS (100\%) \\
\end{longtable}

\subsection{Score Distribution}

\[
  \bar{s} = \frac{20 \times 5 + 1 \times 4 + 1 \times 3}{22} = \frac{107}{22} \approx 4.86
\]
...
\midrule
5/5 & 20 & All except flink\_micro\_commit\_scatter, late\_arriving\_data \\
4/5 &  1 & late\_arriving\_data \\
3/5 &  1 & flink\_micro\_commit\_scatter \\
\midrule
