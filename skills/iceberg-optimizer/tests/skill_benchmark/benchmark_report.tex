...
\hypersetup{
  ...
  pdftitle={Iceberg Optimizer Skill --- Benchmark Report v4},
}
...
\pagestyle{fancy}
\fancyhf{}
\rhead{\textcolor{midgray}{\small Iceberg Optimizer Benchmark Report v4}}
...
\begin{titlepage}
  ...
  {\large Benchmark Report --- Version~4}\\[2em]
  {\large June~2026}\\[4em]
  \begin{tabular}{ll}
    \toprule
    ...
    \textbf{Overall (skill)}    & \pass\ --- 22/22 scenarios, avg judge score 5.0/5 \\
    \textbf{Baseline comparison} & 8 scenarios; skill lifts 3 of them \\
    \bottomrule
  \end{tabular}
\end{titlepage}
...
This report documents the fourth benchmark run: 22 scenarios with a full
skill vs.\ baseline comparison on 8 of them.

Previous runs (v1/v2) achieved 4.8/5 and 5.0/5 on the original five scenarios.
Version~3 expanded coverage to 22 scenarios (avg 4.86/5) and identified two
below-5 cases: \code{flink\_micro\_commit\_scatter} (3/5 --- z-order
erroneously included) and \code{late\_arriving\_data} (4/5 --- K recommended
despite already being set).  Version~4 fixes both via targeted improvements to
the decision framework and fixture data, achieving a \textbf{perfect 5.0/5}
across all 22 scenarios.

Version~4 also introduces a \textbf{baseline comparison} mode
(\texttt{--no-skill} flag) that sends only raw table metadata and
interview answers --- no skill document, no workload analysis, no cost
simulation output.  This isolates what the skill framework actually adds.
Key finding: the skill's value is concentrated in \emph{cross-signal
reasoning} (profile $\times$ workload signals $\times$ user intent~$\to$
action selection), not in general Iceberg domain knowledge which Claude
already has.
...
The harness (\code{run\_benchmark.py}) supports two modes:

\paragraph{Skill mode (default).}
\begin{enumerate}
...
\paragraph{Baseline mode (\texttt{--no-skill}).}
Identical to skill mode except:
\begin{itemize}
  \item The system prompt is a minimal 247-character generic expert prompt
        (\emph{``You are an expert Apache Iceberg data engineer\ldots''}) ---
        no \code{SKILL.md}, no reference documents.
  \item The user message contains \emph{only} \code{profile.json} and the
        scripted Phase~2b interview answers --- no \code{workload.json} and
        no simulator output.
\end{itemize}

This isolates the contribution of the skill framework: workload signal
interpretation, cost simulation, and the decision-framework's cross-signal
reasoning.  The baseline agent must derive everything from raw table metadata
and stated user priorities alone.
...
v4 & Jun 2026 & 22 & 5.0/5 & 22/22 (100\%) + baseline comparison \\
\bottomrule
\end{tabular}
\end{table}
...
The baseline agent receives only \code{profile.json} and interview answers ---
no workload analysis, no cost simulation, no skill document.  The comparison
was run on 8 representative scenarios.

\begin{table}[ht]
\centering
\caption{Skill vs.\ baseline comparison (8 scenarios, LLM-judge scores)}
\begin{tabular}{lcccl}
\toprule
\textbf{Scenario} & \textbf{Baseline} & \textbf{Skill} & \textbf{$\Delta$} & \textbf{Baseline failure mode} \\
\midrule
format\_version\_mismatch        & 5/5 & 5/5 & 0  & --- \\
position\_delete\_accumulation   & 5/5 & 5/5 & 0  & --- \\
gdpr\_deletes                    & 5/5 & 5/5 & 0  & Missing keyword only$^*$ \\
snapshot\_time\_travel\_cdc      & 5/5 & 5/5 & 0  & --- \\
z\_order\_too\_many\_columns     & 5/5 & 5/5 & 0  & --- \\
cdc\_high\_churn\_cow\_consider. & 4/5 & 5/5 & +1 & Did not surface COW mode evaluation \\
flink\_micro\_commit\_scatter    & 4/5 & 5/5 & +1 & Included z-order (no filter-type reasoning) \\
\textbf{late\_arriving\_data}    & \textcolor{failred}{\textbf{2/5 FAIL}} & 5/5 & \textbf{+3} & \textit{(see below)} \\
\midrule
\textbf{Average}                 & 4.38/5 & 5.0/5 & \textbf{+0.63} & \\
\bottomrule
\end{tabular}

\smallskip
{\small $^*$ LLM judge passed (5/5); keyword check failed on \code{remove-dangling-deletes}.}
\end{table}

\subsubsection*{Deep Dive: late\_arriving\_data (baseline 2/5 FAIL)}

The \code{late\_arriving\_data} scenario is the clearest demonstration of
what the skill framework adds.  Without workload signals, the baseline agent:

\begin{itemize}
  \item Recommended \emph{bin-pack} compaction (Action~A) instead of sort
        compaction (Action~B) on recently-modified partitions.
  \item Recommended rebuilding the sort key via CTAS (Action~K analogue),
        despite the table already having \code{has\_sort\_order=true}.
  \item Incorrectly described the existing sort order as \code{event\_id~ASC}
        instead of reading it from the profile (\code{event\_date, customer\_id}).
  \item Did not scope compaction to the 7--30-day late-arrival window --- it
        recommended full-table compaction.
\end{itemize}

The skill fixes all four failures because it reads \code{has\_sort\_order=true}
from \code{profile.json} (skipping K), reads the late\_data flag from
\code{workload.json} (triggering B on affected partitions only), and applies
the K prerequisite check from \code{decision-framework.md}.

\subsubsection*{Interpretation}

The skill's largest lift occurs where \textbf{cross-signal reasoning} is
required: combining profile metadata + workload signals + user intent to
select the correct action among multiple plausible alternatives.

For scenarios where a single strong signal dominates (format version mismatch,
GDPR ordering, time math error, z-order collapse), the baseline already scores
5/5 --- the knowledge is in Claude's training data and raw metadata is
sufficient.

For scenarios where the correct action depends on combining \emph{multiple
weak signals} (late arrival + existing sort order + affected partitions only;
Flink thin spread + equality-only predicates; CDC + 12~MB writes + COW
evaluation), the skill framework provides the structured reasoning path that
the baseline misses.
...
\paragraph{v4 improvements (this report).}
...
\begin{enumerate}
  ...
  \item \textbf{K prerequisite check (late\_arriving 4/5 $\to$ 5/5).}
...
\begin{enumerate}
  ...
  \item \textbf{[Low] Expand baseline comparison to all 22 scenarios.}  The 8-
        scenario sample already shows the pattern clearly, but a full 22-scenario
        baseline run would give a definitive lift measurement.
\end{enumerate}
...
# Full suite (skill mode)
python run_benchmark.py --all --judge --output-json results.json

# Baseline comparison (profile.json + interview answers only; no skill)
python run_benchmark.py --scenario late_arriving_data --judge --no-skill
python run_benchmark.py --all --judge --no-skill --output-json baseline.json
...
Version~4 of the iceberg-optimizer benchmark achieves a \textbf{perfect 5.0/5}
across all 22 scenarios (up from 4.86/5 in v3), and introduces the first
systematic baseline comparison showing where the skill framework adds measurable
value.

\paragraph{Score improvements from v3 to v4.}
Both below-5 v3 scenarios were fixed via generalised decision-framework updates:
\begin{itemize}
...
\paragraph{Baseline comparison findings.}
The skill's largest lifts occur in \emph{cross-signal} scenarios where the
correct action requires combining multiple weak signals from profile, workload,
and intent data:
\begin{itemize}
  \item \code{late\_arriving\_data}: baseline 2/5 FAIL $\to$ skill 5/5 (+3) ---
        the baseline defaults to bin-pack (wrong) and redundantly recommends K
        (already set).  The skill reads \code{has\_sort\_order} and the
        late\_data flag to prescribe targeted sort compaction.
  \item \code{flink\_micro\_commit\_scatter}: baseline 4/5 $\to$ skill 5/5
        (+1) --- without workload filter-type analysis, the baseline cannot
        determine that equality-only predicates make z-order wrong.
  \item \code{cdc\_high\_churn}: baseline 4/5 $\to$ skill 5/5 (+1) --- the
        COW mode evaluation requires combining write-size and query-rate signals.
\end{itemize}

For scenarios dominated by a single strong signal (format version, GDPR
ordering, count vs time math, z-order collapse), the baseline already reaches
5/5 --- the skill adds nothing because Claude's training data contains the
relevant knowledge and the raw profile alone is sufficient.
